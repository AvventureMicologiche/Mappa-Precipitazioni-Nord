...
- **Centratura misurata su telefono + pannello cumulati compatto** (6 agosto 2026, prod e test): la pillola canale ha allungato il pannello dei cumulati e tutte le regioni della tendina finivano troppo in basso (le viste erano tarate a mano sul pannello di prima — segnalato su Svizzera e Liguria). Due interventi: (a) su ≤600px statistiche, gradiente e pillola compattati (`.sv` 26→20px, `.grad` 18→14, avatar 46→38; ~25px recuperati); (b) `centraRegioneInStriscia()` — si MISURA quanto i pannelli (`legend-panel`/`time-panel`) coprono la mappa e si sposta la regione al centro della striscia visibile, invece di ritarare 13 centri a occhio. Chiamata 420ms dopo la selezione dalla tendina e 350ms dopo `showLegendMobile` (i due pannelli hanno altezze diverse); non tocca nulla se nessun pannello copre la mappa (desktop/tablet coi pannelli laterali) o se lo scarto è ≤12px. Verificata headless su TUTTE le regioni a 375×812: delta dal centro striscia ≤±25px (l'unico ⚠️ apparente è la Sicilia, ma è l'effetto delle isole nel confine disegnato — la parte continentale è centrata). Sopravvive a future modifiche di altezza dei pannelli.
...
**Aperto, trovato il 5/8**: il **29 e 30 giugno** ARPAE è molto più bassa di OMIRL sulle gemelle (37,7 contro 88,5 e 7,0 contro 35,2). Sono giorni **dopo** il 19 giugno, quindi lì il metro è valido e la divergenza va spiegata; gli altri cinque giorni bagnati dello stesso periodo sono identici al decimale, quindi non è sistematico.
