...
- **Collector v2 IN PROVA nel repo di TEST dal 17/8/2026** (`piemonte.yml` aggiunto lì con `permissions: contents: write`): ricalcola D-1..D-3 dal realtime con campo `h` = ore valide (merge «vince chi ha più ore» al posto del MAX), passata sulla **banca dati storica ufficiale** `utility.arpa.piemonte.it/meteoidro/dati_giornalieri_meteo/?data_min=G&data_max=G&page_size=1000` (una chiamata per tutte le stazioni, disponibile già per ieri): stazioni assenti dal realtime aggiunte SOLO se ptot>0 (gli zeri ufficiali non sono affidabili senza realtime: Omegna Lago d'Orta ad agosto scrive 0 col sensore fermo), realtime incompleto (h<24, |Δ|>1 mm) sostituito, t/w riempiti dove mancano (**vento ufficiale in m/s ×3,6**), **quota `q` da `stazione_meteorologica`** (join per `codice_stazione` = station_code, 273/273) su tutte. ⚠️ **L'archivio ufficiale conta il giorno in UTC** (giorno G = record locali 03:00 di G → 02:00 di G+1 in ora legale, verificato su Crodo/Druogno/Cursolo): NON si adotta, il progetto usa il giorno solare italiano; le differenze da giorno UTC (pioggia 00-02) si loggano soltanto. `DRY_RUN=1` per provare. Da promuovere in prod (solo `.github/`, zero deploy) dopo qualche run di cron. Confronto ufficiale↔nostro 1/7-13/8: 97,6% entro 1 mm, somma −0,4%.
