...
(function(){
  ...
  function disegna(k, d){
    ...
    if (!d.top || !d.top.length || d.top[0].mm < 1) {
      top.innerHTML = '<p class="nota">' + (k === '1' ? 'Giornata' : 'Periodo') + ' quasi asciutto in tutta la zona.</p>';
    } else {
    ...
    if (k === '1') {
      ...
      nota.textContent = 'Dal ' + breve(d.primo) + ' al ' + breve(d.ultimo) + ', su ' + d.giorni + ' giornate di dati. La giornata odierna è esclusa.';
    }
