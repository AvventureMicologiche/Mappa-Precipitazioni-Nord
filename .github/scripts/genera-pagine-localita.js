...
.tt{width:100%;height:150px;margin:14px 0 4px;display:block;}
.tt .griglia{stroke:#e3e8ef;stroke-width:1;vector-effect:non-scaling-stroke;}
.tt .max{fill:none;stroke:#d1603d;stroke-width:2.2;stroke-linejoin:round;stroke-linecap:round;vector-effect:non-scaling-stroke;}
.tt .min{fill:none;stroke:#3a6ea8;stroke-width:2.2;stroke-linejoin:round;stroke-linecap:round;vector-effect:non-scaling-stroke;}
.tt .et{fill:#6b7a8d;font-size:10.5px;}
.vv .b i{background:#8fa9c4;}
  ...
  function disegna(j){
    ...
    if (sT || sW) {
      ...
      if (sT) {
        ...
        /* linea spezzata sui buchi: un giorno senza dato non si inventa */
        var linea = function (idx) {
          var pezzi = [], cur = [];
          for (var q2 = GIORNI; q2 >= 1; q2--) {
            var x = (GIORNI - q2) * passo, d = sT[q2 - 1];
            if (d) cur.push(x.toFixed(1) + ',' + y(d[idx]).toFixed(1));
            else { if (cur.length > 1) pezzi.push(cur.join(' ')); cur = []; }
          }
        ...
        for (var g2 = 0; g2 <= 4; g2++) {
          var yy = (g2 * HH / 4).toFixed(1);
          svg += '<line class="griglia" x1="0" y1="' + yy + '" x2="100" y2="' + yy + '"/>';
        }
        for (var k2 = 0; k2 < 2; k2++)
          svg += linea(k2).map(function (p) { return '<polyline class="' + cl[k2] + '" points="' + p + '"/>'; }).join('');
        H += '<svg class="tt" viewBox="0 0 100 100" preserveAspectRatio="none">' + svg + '</svg>'
          + '<div class="gg-x">' + ascisse + '</div>'
          + '<p class="gg-leg"><i style="background:#d1603d"></i>massima &nbsp;'
          + '<i style="background:#3a6ea8"></i>minima &nbsp; da ' + lo + ' a ' + hi + ' &deg;C</p>';
      }
      if (sW) {
        var maxW = Math.max.apply(null, sW.filter(function (v) { return v != null; }).concat([1]));
        var bw = '';
        for (var i2 = GIORNI; i2 >= 1; i2--) {
          var vv = sW[i2 - 1];
          var dd = menoDa(j.oggi, i2);
          bw += '<div class="b" title="' + gg(iso(dd)) + ': ' + (vv == null ? 'nessun dato' : num(vv) + ' km/h') + '">'
            + (vv == null ? '' : '<i style="height:' + Math.max(2, Math.round(vv / maxW * 100)) + '%"></i>') + '</div>';
        }
        H += '<h3 style="font-size:16px;margin:22px 0 0">Vento medio</h3>'
          + '<div class="gg vv" style="height:90px">' + bw + '</div>'
          + '<div class="gg-x">' + ascisse + '</div>'
          + '<p class="gg-leg">media del giorno in km/h, massimo del periodo ' + num(maxW) + '</p>';
      }
